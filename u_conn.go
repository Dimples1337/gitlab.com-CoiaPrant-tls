...
func UServer(conn net.Conn, config *Config) *UConn {
	if config == nil {
		config = &Config{}
	}
	tlsConn := Conn{conn: conn, config: config, isClient: true}
...
func (c *UConn) GetClientHello() (clientHello *ClientHelloMsg, err error) {
	if c.handshakeComplete() {
		err = errors.New("tls.Handshake completed")
		return
	}
	...
	if c.isClient {
		...
		clientHello = c.HandshakeState.Hello
	} else {
		if c.clientHello != nil {
			clientHello = c.clientHello.getPublicPtr()
			return
		}

		c.clientHello, err = c.readClientHello(context.Background())
		if err != nil {
			return
		}
...
func (c *UConn) serverHandshake(ctx context.Context) error {
	// If this is the first server handshake, we generate a random key to
	// encrypt the tickets with.
	_, err := c.GetClientHello()
	if err != nil {
		return err
	}
