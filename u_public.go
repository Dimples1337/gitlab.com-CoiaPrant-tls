...
func (pis pskIdentities) ToPublic() []PskIdentity {
	var PIS []PskIdentity
	for _, pi := range pis {
		PIS = append(PIS, PskIdentity{Label: pi.label, ObfuscatedTicketAge: pi.obfuscatedTicketAge})
	}
	return PIS
}
func (PIS PskIdentities) ToPrivate() []pskIdentity {
	var pis []pskIdentity
	for _, PI := range PIS {
		pis = append(pis, pskIdentity{label: PI.Label, obfuscatedTicketAge: PI.ObfuscatedTicketAge})
	}
